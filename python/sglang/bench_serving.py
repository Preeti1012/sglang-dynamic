...
def get_dataset(args, tokenizer):
    ...
    elif args.dataset_name == "custom_shared_prefix_dataset":
        input_requests = sample_custom_shared_prefix_requests_with_sysprompt(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
...
# def sample_custom_shared_prefix_requests1(
#     dataset_path: str,
#     num_requests: int,
#     tokenizer: PreTrainedTokenizerBase,
...
# ) -> List[DatasetRow]:
#     """
#     Load and sample from your custom custom shared-prefix dataset.
#     Expected JSON format:
#     [
#         {"system": "shared prefix text ...", "user": "question 1 ..."},
#         {"system": "shared prefix text ...", "user": "question 2 ..."},
#         ...
#     ]
#     """
...
#         # Combine shared prefix + question
#         prompt = (system.strip() + "\n\n" + user.strip()).strip()
...
#         # Default completion length: same as sharegpt or use fixed_output_len
#         output_len = fixed_output_len if fixed_output_len is not None else 128

#         # Prune too short/too long samples
#         if prompt_len < 2 or (context_len and prompt_len + output_len > context_len):
#             continue
...
#     print(f"Loaded {len(filtered_dataset)} valid samples from {dataset_path}")

#     return filtered_dataset

# def sample_custom_shared_prefix_requests2(
#     dataset_path: str,
#     num_requests: int,
#     tokenizer: PreTrainedTokenizerBase,
#     fixed_output_len: Optional[int] = None,
...
# ) -> List[DatasetRow]:
#     """
#     Load and sample from custom shared-prefix dataset.

#     Expected JSON format:
#     [
#         {"id": 0, "user": "question 1 ..."},
#         {"id": 1, "user": "question 2 ..."},
#         ...
#     ]

#     The shared prefix (system prompt) will be provided separately at runtime.
#     """
#     import json, random, numpy as np
...
#         # The shared prefix will be sent separately by the benchmarker,
#         # so this prompt only includes the user question.
#         prompt = user.strip()
...
#     print(f"#Input tokens: {np.sum([x.prompt_len for x in filtered_dataset])}")
#     print(f"#Output tokens: {np.sum([x.output_len for x in filtered_dataset])}")
#     print(f"Loaded {len(filtered_dataset)} valid user-only prompts from {dataset_path}")

#     return filtered_dataset

# def sample_custom_shared_prefix_requests3(
#     dataset_path: str,
#     num_requests: int,
#     tokenizer: PreTrainedTokenizerBase,
#     fixed_output_len: Optional[int] = None,
...
#     If num_requests > len(dataset), the dataset will automatically
#     cycle (repeat) until num_requests is reached.
#     """
#     import json, random, numpy as np
...
#     # Cycle through dataset until we reach num_requests
#     for i, item in enumerate(cycle(dataset)):
#         if len(filtered_dataset) >= num_requests:
#             break
...
#         # The shared prefix will be added separately
#         prompt = user.strip()
...
#         # Skip invalid or too long samples
#         if prompt_len < 2 or (context_len and prompt_len + output_len > context_len):
#             continue
...
#     print(f"#Requested samples: {num_requests}")
#     print(f"#Generated samples (cycled): {len(filtered_dataset)}")
#     print(f"#Input tokens: {np.sum([x.prompt_len for x in filtered_dataset])}")
#     print(f"#Output tokens: {np.sum([x.output_len for x in filtered_dataset])}")
...
# def sample_custom_shared_prefix_requests_with_sysprompt(
#     dataset_path: str,
#     num_requests: int,
#     num_families: int,
#     type_of_dataset: int,
...
#     """
#     Load and sample from custom shared-prefix dataset.
#     Each family (aa, ab, ...) gets num_requests / num_families samples,
#     cycling through dataset questions if needed.

#     The system prompt for each family is read from `system_prompt.py`
#     and prefixed with the family label (e.g., 'aa You are an OS expert...').
#     """
#     import json
#     import random
#     import string
...
#     from system_prompt import system_prompt  # <-- import your shared system prompt text
...
#     num_fam = num_families  # default 10 change accordingly ******************************************************
#     family_size = num_requests // num_fam
#     # family_size = fam_size
#     filtered_dataset: List[DatasetRow] = []

#     # --- Generate 2-letter prefixes ---
#     prefix_list = [a + b for a, b in product(string.ascii_lowercase, repeat=2)]
#     if num_fam > len(prefix_list):
#         raise ValueError(f"Too many families ({num_fam}); max is {len(prefix_list)}")

#     # --- Main loop per family ---
#     for fidx in range(num_fam):
#         prefix = prefix_list[fidx]
#         print(f"Generating family {prefix} with {family_size} samples")

#         # Family-specific system prompt
#         modified_system_prompt = f"{prefix} {system_prompt.strip()}"

#         # Cycle through dataset repeatedly to reach family_size
#         for i, item in enumerate(islice(cycle(dataset), family_size)):
#             user = item.get("user", "")
#             if not user.strip():
#                 continue
...
#                 prompt = prompt.replace(tokenizer.bos_token, "")
#             else:
#                 # --- Plain concatenation mode ---
#                 prompt = f"{modified_system_prompt}\n\n{user.strip()}"
#                 if prompt_suffix:
#                     prompt += prompt_suffix
...
def sample_custom_shared_prefix_requests_with_sysprompt(
    ...
    """
    Generate num_requests requests with:
      - num_families families
      - family batches of size fam_size
      - sequential family batching (aa, aa, aa, ab, ab, ab, ...)
      - ensuring each family gets at least 1 full batch
      - custom batch distribution rule:
            Example: 22 batches, 5 families => [5,5,5,5,2]
    """
    ...
    # --------------------------
    # Compute total batches required
    # --------------------------
    total_batches = num_requests // fam_size

    # If not enough to give each family 1 full batch, increase it
    if total_batches < num_families:
        total_batches = num_families
        num_requests = fam_size * total_batches  # adjust upward

    # --------------------------
    # Custom batch distribution
    # --------------------------
    def distribute_batches(B, F):
        base = B // F
        rem = B % F

        batches = [base] * F

        # Give +1 to first families (except last)
        for i in range(min(rem, F - 1)):
            batches[i] += 1

        # Spillover remainder (if any) goes to **last family**
        if rem > (F - 1):
            batches[-1] += rem - (F - 1)

        return batches

    batches_per_family = distribute_batches(total_batches, num_families)
    print("Batch distribution per family:", batches_per_family)

    # --------------------------
    # Prefix list for families
    # --------------------------
    prefix_list = [a + b for a, b in product(string.ascii_lowercase, repeat=2)]
    if num_families > len(prefix_list):
        raise ValueError(f"Too many families ({num_families}); max is {len(prefix_list)}")

    filtered_dataset = []

    # --------------------------
    # Generate requests family-by-family
    # --------------------------
    for fidx in range(num_families):
        prefix = prefix_list[fidx]
        n_batches = batches_per_family[fidx]
        n_requests = n_batches * fam_size

        print(f"Generating {n_requests} requests for family {prefix}")
        ...
        while count < n_requests:
            ...
            prompt_token_ids = tokenizer.encode(prompt)
            prompt_len = len(prompt_token_ids)
            output_len = fixed_output_len if fixed_output_len else 128
    ...
    print(f"#Input tokens: {np.sum([x.prompt_len for x in filtered_dataset])}")
    print(f"#Output tokens: {np.sum([x.output_len for x in filtered_dataset])}")
    print(f"Generated {len(filtered_dataset)} valid samples.")

    return filtered_dataset


def sample_custom_shared_prefix_requests_without_sysprompt(
    dataset_path: str,
    num_requests: int,
    tokenizer,
    fixed_output_len: int = None,
    ...
    """
    Load and sample from MLFQ shared-prefix dataset.
    Each family (aa, ab, ...) gets num_requests / num_families samples,
    cycling through dataset questions if needed.
    """
    import json
    ...
    dataset_size = len(dataset)
    num_families = 50 # change it accordingly************************************************************************
    family_size = num_requests // num_families
    filtered_dataset: List[DatasetRow] = []
    ...
    random.shuffle(filtered_dataset) # Uncomment if you want to shuffle the final dataset *******************************

    # for prompt in filtered_dataset:
    #     print(prompt)
...
if __name__ == "__main__":
    ...
    # Customized argument for number of families
    parser.add_argument(
        "--num-families",
        type=int,
        default=10,
        help="Number of families. Default is 10.",
    )
    # Customized argument for random or ordered custom dataset
    parser.add_argument(
        "--type-of-dataset",
        type=int,
        default=1,
        help="Type of dataset: random or order. Default is 1.",
    )
    parser.add_argument(
        "--fam-size",
        type=int,
        default=1,
        help="Family Size. Default is 1.",
    )
    parser.add_argument(
        "--sharegpt-output-len",
        type=int,
