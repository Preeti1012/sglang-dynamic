...
class Req:
    ...
    def init_next_round_input(
        ...
        # logger.info(f"##### init_next_round_input len(self.fill_ids): {len(self.fill_ids)} len(self.prefix_indices): {len(self.prefix_indices)} #####")
    ...
    def adjust_max_prefix_ids(self):
        ...
        # logger.info(f"##### adjust_max_prefix_ids max_prefix_len: {max_prefix_len} input_len: {input_len} #####")
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def init_new(
        ...
        # logger.info(f"##### ScheduleBatch init_new with {len(reqs)} reqs #####")
    ...
    def batch_size(self):
        # logger.info(f"##### ScheduleBatch batch_size: {len(self.reqs)} #####")
    ...
    def alloc_token_slots(self, num_tokens: int, backup_state: bool = False):
        # logger.info(f"##### ScheduleBatch alloc_token_slots: {num_tokens} #####")
    ...
    def prepare_for_decode(self):
        # logger.info("##### ScheduleBatch prepare_for_decode #####")
    ...
    def _evict_tree_cache_if_needed(self, num_tokens: int):
        if isinstance(self.tree_cache, (SWAChunkCache, ChunkCache)):
            # logger.info(f"##### ScheduleBatch _evict_tree_cache_if_needed for {num_tokens} num_tokens tokens. tree_cache is instance of (SWAChunkCache, ChunkCache) #####\n")
            return
        # logger.info(f"##### ScheduleBatch _evict_tree_cache_if_needed for {num_tokens} num_tokens tokens. tree_cache is not instance of (SWAChunkCache, ChunkCache) #####\n")

        if self.is_hybrid:
            full_available_size = self.token_to_kv_pool_allocator.full_available_size()
            swa_available_size = self.token_to_kv_pool_allocator.swa_available_size()
            # logger.info(f"##### ScheduleBatch _evict_tree_cache_if_needed is_hybrid: full_available_size {full_available_size}, swa_available_size {swa_available_size} #####\n")

            if full_available_size < num_tokens or swa_available_size < num_tokens:
                if self.tree_cache is not None:
                    full_num_tokens = max(0, num_tokens - full_available_size)
                    swa_num_tokens = max(0, num_tokens - swa_available_size)
                    # logger.info(f"##### ScheduleBatch _evict_tree_cache_if_needed is_hybrid: full_available_size < num_tokens: full_available_size {full_available_size}, swa_available_size {swa_available_size} #####\n")
                    self.tree_cache.evict(full_num_tokens, swa_num_tokens)
        else:
            # logger.info(f"##### ScheduleBatch _evict_tree_cache_if_needed is_not_hybrid: self.token_to_kv_pool_allocator.available_size {self.token_to_kv_pool_allocator.available_size()} #####\n")
    ...
    def _available_and_evictable_str(self) -> str:
        if self.is_hybrid:
            # logger.info(f"##### ScheduleBatch _available_and_evictable_str is_hybrid: available_size {available_size}, evictable_size {evictable_size} #####\n")
            full_available_size = self.token_to_kv_pool_allocator.full_available_size()
            swa_available_size = self.token_to_kv_pool_allocator.swa_available_size()
            ...
            # logger.info(f"##### ScheduleBatch _available_and_evictable_str: available_size {available_size}, evictable_size {evictable_size} #####\n")
