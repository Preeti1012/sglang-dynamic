...
class KVCache(abc.ABC):
    ...
    def _finalize_allocation_log(self, num_tokens: int):
        ...
        if isinstance(kv_size_bytes, tuple):
            ...
            v_size_GB = v_size / GB
            logger.info(
                f"KV Cache is allocated. #tokens: {num_tokens}, K size: {k_size_GB:.2f} GB, V size: {v_size_GB:.2f} GB, self.store_dtype: {self.store_dtype} layer_num: {self.layer_num}, self.size (in_seq_len): {self.size}, self.page_size: {self.page_size}"
            )
...
class MHATokenToKVPool(KVCache):
    ...
    def get_kv_size_bytes(self):
        ...
        # logger.info(f"##### memory_pool: KV Cache size: K={k_size_bytes} Bytes, V={v_size_bytes} Bytes k_cache.dtype.itemsize: {k_cache.dtype.itemsize} #####")
        # logger.info(f"##### k_cache shapes: {[k_cache.shape for k_cache in self.k_buffer]} #####")
        # logger.info(f"##### v_cache shapes: {[v_cache.shape for v_cache in self.v_buffer]} #####")
    ...
    def get_contiguous_buf_infos(self):
        ...
        # logger.info(f"##### memory_pool get_contiguous_buf_infos: kv_data_ptrs={kv_data_ptrs}, kv_data_lens={kv_data_lens}, kv_item_lens={kv_item_lens} #####")
